...
/** แถบเมนูด้านล่างสำหรับมือถือ ซ่อนบน md+ เพราะมี SiteHeader แสดงเมนูแล้ว */
export function BottomNav() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-background/95 backdrop-blur supports-backdrop-filter:bg-background/80 md:hidden">
      <div className="grid grid-cols-6 safe-area-inset-bottom">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <Icon className={cn("size-5 shrink-0", isActive && "text-brand-gold-hover")} />
