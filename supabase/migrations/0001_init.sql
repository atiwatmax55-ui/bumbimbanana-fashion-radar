...
-- ตาราง products: เก็บข้อมูลสินค้าแฟชั่นที่ดึงมาจากแหล่งข้อมูล (Mock Data หรือ Windsor.ai ในอนาคต)
...
comment on table public.products is 'สินค้าแฟชั่นจาก TikTok Shop (ปัจจุบันเป็น Mock Data ในแอป ยังไม่ได้ซิงก์เข้าตารางนี้จริง)';
...
comment on table public.saved_products is 'สินค้าที่บันทึกไว้สนใจทำคอนเทนต์ (เวอร์ชันแอปปัจจุบันเก็บข้อมูลนี้ใน local storage ฝั่งเบราว์เซอร์ ยังไม่เขียนตารางนี้จริง)';

create index if not exists idx_saved_products_product_id on public.saved_products (product_id);

-- ตาราง data_sync_logs: บันทึกประวัติการซิงก์ข้อมูลจากแหล่งข้อมูลภายนอก (เช่น Windsor.ai ในอนาคต)
...
comment on table public.data_sync_logs is 'ประวัติการซิงก์ข้อมูลสินค้าจากแหล่งข้อมูลภายนอก เตรียมไว้สำหรับเชื่อมต่อ Windsor.ai ในอนาคต';
